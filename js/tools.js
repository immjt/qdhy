...
var MapToolUI = function () {
 ...
 //   "        <li id='btnExtent' class='btnToolbar' style='margin-right: 20px;'><span class='btnToolbar-cont'><span class='btnToolbar-img' style='background-position: -128px 0;'></span>全屏</span></li>",
    ...
    "           <div id='toolbag_div' style='width: 86px; border: 1px solid #25abf3; background: #f4f9fd;display: none; color: #777777;margin-top:24px;'>",
    "               <ul></ul>",
    "        </li>",
    ...
    "        <li id='btnheat' class='btnToolbar'><span class='btnToolbar-cont'><span class='btnToolbar-img' style='background-position: -96px 0;'></span><span id='span_title'>打开热力图</span></span></li>",
    "        <li id='vec_img'></li>",
    "    </ul>"].join("");
    var inited = false;
    return {
        init: function (sdmap) {
            ...
            //显示热力图
            var bz_heat=0;//0close
            $('#btnheat').click(function () {
                var p={
                    id:"heat_ly",
                    url:Service.heatTile,
                    layer:"0"
                };
                if(bz_heat==0)
                {
                    wmswmts.addLayer(p);
                    bz_heat=1;
                    $('#span_title').html("关闭热力图");
                }
                else{wmswmts.removeLayer(p.id);bz_heat=0;$('#span_title').html("打开热力图");}
            });
